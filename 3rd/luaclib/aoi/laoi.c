...
static int lua__map_new(lua_State *L)
{
	...
	divide = (int)luaL_optnumber(L, 2, 1);
...
{
	imap * map = CHECK_AOI_MAP(L, 1);
	int require = (int)luaL_optnumber(L, 2, EnumNodePrintStateAll);
...
static int lua__map_dumpstate(lua_State *L)
{
	...
	int require = (int)luaL_optnumber(L, 4, EnumMapStateAll);
...
static int lua__map_unit_add(lua_State *L)
{
	...
	DLOG("unit_add *unit=%p,unit(%f, %f)\n", unit, unit->pos.x, unit->pos.y);

	luac__map_getfield(L, 1, AOI_UNITS_MAP_NAME);
	lua_pushinteger(L, (lua_Integer)unit->id);
	lua_rawget(L, -2);

	// luac_dumpstack(L, stderr);
	if (lua_isnil(L, -1)) {
...
static int lua__unit_new(lua_State *L)
{
	...
	DLOG("new unit,id=%lld,unit=%p,gettop=%d\n", id, u, lua_gettop(L));
	return 1;
}
