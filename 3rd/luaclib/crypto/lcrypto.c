...
static void luacrypto_register_submodule(lua_State * L, const char *libname,
					 const luaL_Reg * l)
{
...
{
	EVP_MD_CTX *c =
	    (EVP_MD_CTX *) luaL_checkudata(L, 1, LUACRYPTO_DIGESTNAME);
...
static int parse_enc_params(lua_State * L, EVP_CIPHER ** cipher, char **key,
			    size_t * key_len, char **iv, size_t * iv_len,
			    int *pad, int *size_to_return, int cipher_pos,
			    int key_pos, int iv_pos, int pad_pos)
{
...
static int parse_new_enc_params(lua_State * L, EVP_CIPHER ** cipher, char **key,
				size_t * key_len, char **iv, size_t * iv_len,
				int *pad, int *size_to_return)
{
	return parse_enc_params(L, cipher, key, key_len, iv, iv_len, pad,
				size_to_return, 1, 2, 3, 4);
}

static int parse_f_enc_params(lua_State * L, EVP_CIPHER ** cipher, char **key,
			      size_t * key_len, char **iv, size_t * iv_len,
			      int *pad, int *size_to_return)
{
	return parse_enc_params(L, cipher, key, key_len, iv, iv_len, pad,
				size_to_return, 2, 4, 5, 6);
}

#define TRY_CTX(fun) if (!fun) { *size_to_return = crypto_error(L); return 0; }

static int
init_encryptor_decryptor(int (*init_fun)
			 (EVP_CIPHER_CTX *, const EVP_CIPHER *, ENGINE *,
			  const unsigned char *, const unsigned char *),
			 lua_State * L, EVP_CIPHER_CTX * c,
			 const EVP_CIPHER * cipher, const char *key,
			 size_t key_len, const char *iv, size_t iv_len, int pad,
			 int *size_to_return)
{
...
{
	EVP_CIPHER_CTX *c =
	    (EVP_CIPHER_CTX *) lua_newuserdata(L, sizeof(EVP_CIPHER_CTX));
...
static int encrypt_fnew(lua_State * L)
{
	...
	if (!parse_new_enc_params
	    (L, (EVP_CIPHER **) & cipher, (char **)&key, &key_len, (char **)&iv,
	     &iv_len, &pad, &size_to_return)) {
		return size_to_return;
	}

	c = encrypt_pnew(L);
	if (!init_encryptor_decryptor
	    (EVP_EncryptInit_ex, L, c, cipher, key, key_len, iv, iv_len, pad,
	     &size_to_return)) {
		return size_to_return;
	}
	return 1;
...
{
	EVP_CIPHER_CTX *c =
	    (EVP_CIPHER_CTX *) luaL_checkudata(L, 1, LUACRYPTO_ENCRYPTNAME);
	size_t input_len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 2, &input_len);
	int output_len = 0;
	unsigned char *buffer = NULL;

	buffer =
	    (unsigned char *)malloc(input_len +
				    (size_t) EVP_CIPHER_CTX_block_size(c));
...
static int encrypt_fencrypt(lua_State * L)
{
	...
	size_t input_len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 3, &input_len);
	...
	if (!parse_f_enc_params
	    (L, (EVP_CIPHER **) & type, (char **)&key, &key_len, (char **)&iv,
	     &iv_len, &pad, &size_to_return)) {
		return size_to_return;
	}

	if (!init_encryptor_decryptor
	    (EVP_EncryptInit_ex, L, &c, type, key, key_len, iv, iv_len, pad,
	     &size_to_return)) {
		return size_to_return;
	}
...
static int decrypt_fnew(lua_State * L)
{
	...
	c = decrypt_pnew(L);
	if (!init_encryptor_decryptor
	    (EVP_DecryptInit_ex, L, c, cipher, key, key_len, iv, iv_len, pad,
	     &size_to_return)) {
		return size_to_return;
	}
	return 1;
...
{
	EVP_CIPHER_CTX *c =
	    (EVP_CIPHER_CTX *) luaL_checkudata(L, 1, LUACRYPTO_DECRYPTNAME);
	size_t input_len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 2, &input_len);
	int output_len = 0;
...
static int decrypt_fdecrypt(lua_State * L)
{
	...
	if (!init_encryptor_decryptor
	    (EVP_DecryptInit_ex, L, &c, type, key, key_len, iv, iv_len, pad,
	     &size_to_return)) {
		return size_to_return;
	}
...
{
	EVP_MD_CTX *c =
	    (EVP_MD_CTX *) luaL_checkudata(L, 1, LUACRYPTO_SIGNNAME);
	size_t input_len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 2, &input_len);
...
static int sign_final(lua_State * L)
{
	...
	unsigned char *buffer;
	EVP_PKEY **pkey =
	    (EVP_PKEY **) luaL_checkudata(L, 2, LUACRYPTO_PKEYNAME);
...
static int sign_fsign(lua_State * L)
{
	...
	if (type == NULL) {
		...
		unsigned char *buffer = NULL;
		EVP_PKEY **pkey =
		    (EVP_PKEY **) luaL_checkudata(L, 4, LUACRYPTO_PKEYNAME);
...
{
	EVP_MD_CTX *c =
	    (EVP_MD_CTX *) luaL_checkudata(L, 1, LUACRYPTO_VERIFYNAME);
	size_t input_len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 2, &input_len);
...
static int verify_final(lua_State * L)
{
	...
	const unsigned char *sig =
	    (unsigned char *)luaL_checklstring(L, 2, &sig_len);
	EVP_PKEY **pkey =
	    (EVP_PKEY **) luaL_checkudata(L, 3, LUACRYPTO_PKEYNAME);
	int ret;
...
static int verify_fverify(lua_State * L)
{
	...
	if (type == NULL) {
		...
		size_t sig_len = 0;
		const unsigned char *sig =
		    (unsigned char *)luaL_checklstring(L, 4, &sig_len);
		EVP_PKEY **pkey =
		    (EVP_PKEY **) luaL_checkudata(L, 5, LUACRYPTO_PKEYNAME);
		int ret;
...
static int pkey_generate(lua_State * L)
{
	...
	if (idx == 0) {
		...
		*pkey = EVP_PKEY_new();
		...
		DSA *dsa =
		    DSA_generate_parameters(key_len, NULL, 0, NULL, NULL, NULL,
					    NULL);
	...
	if (idx == 0) {
		...
		if (rsa && e) {
			...
			if (i == e_len) {
				...
				success =
				    RSA_generate_key_ex(rsa, key_len, e, &cb);
			}
		}
		...
		if (dsa) {
			...
			if (DSA_generate_parameters_ex
			    (dsa, key_len, NULL, 0, NULL, NULL, &cb))
...
{
	EVP_PKEY **pkey =
	    (EVP_PKEY **) luaL_checkudata(L, 1, LUACRYPTO_PKEYNAME);
	...
	if (private && pkey_st->type == EVP_PKEY_DSA)
		ret =
		    PEM_write_bio_DSAPrivateKey(mem, pkey_st->pkey.dsa, NULL,
						NULL, 0, NULL, NULL);
	else if (private && pkey_st->type == EVP_PKEY_RSA)
		ret =
		    PEM_write_bio_RSAPrivateKey(mem, pkey_st->pkey.rsa, NULL,
						NULL, 0, NULL, NULL);
	else if (private)
		ret =
		    PEM_write_bio_PrivateKey(mem, *pkey, NULL, NULL, 0, NULL,
					     NULL);
	else
...
static int pkey_tostring(lua_State * L)
{
	EVP_PKEY **pkey =
	    (EVP_PKEY **) luaL_checkudata(L, 1, LUACRYPTO_PKEYNAME);
	char buf[60];
	sprintf(buf, "%s %s %d %p", LUACRYPTO_PKEYNAME,
		(*pkey)->type == EVP_PKEY_DSA ? "DSA" : "RSA",
		EVP_PKEY_bits(*pkey), pkey);
...
{
	seal_context *c =
	    (seal_context *) lua_newuserdata(L, sizeof(seal_context));
...
{
	seal_context *c =
	    (seal_context *) luaL_checkudata(L, 1, LUACRYPTO_SEALNAME);
...
static int seal_tostring(lua_State * L)
{
	seal_context *c =
	    (seal_context *) luaL_checkudata(L, 1, LUACRYPTO_SEALNAME);
	char s[64];
	sprintf(s, "%s %p %s", LUACRYPTO_SEALNAME, (void *)c,
		EVP_CIPHER_name(c->ctx->cipher));
	lua_pushstring(L, s);
	return 1;
}
...
static int seal_fnew(lua_State * L)
{
	...
	seal_ctx->ek =
	    (unsigned char *)malloc((size_t) EVP_PKEY_size(*pkey) *
				    (size_t) npubk);

	if (!EVP_SealInit
	    (seal_ctx->ctx, cipher, &seal_ctx->ek, &seal_ctx->eklen,
...
static int seal_update(lua_State * L)
{
	...
	unsigned char *temp =
	    (unsigned char *)malloc(input_len +
				    (size_t) EVP_CIPHER_CTX_block_size(c->ctx));
...
static int seal_final(lua_State * L)
{
	...
	lua_pushlstring(L, (const char *)c->iv,
			(size_t) EVP_CIPHER_iv_length(c->ctx->cipher));
...
static int seal_fseal(lua_State * L)
{
	...
	ek = (unsigned char *)malloc((size_t) EVP_PKEY_size(*pkey) *
				     (size_t) npubk);
	...
	lua_pushlstring(L, (const char *)iv,
			(size_t) EVP_CIPHER_iv_length(cipher));
...
{
	open_context *c =
	    (open_context *) lua_newuserdata(L, sizeof(open_context));
...
{
	open_context *c =
	    (open_context *) luaL_checkudata(L, 1, LUACRYPTO_OPENNAME);
	EVP_PKEY **pkey =
	    (EVP_PKEY **) luaL_checkudata(L, -1, LUACRYPTO_PKEYNAME);
	char s[64];

	lua_rawgeti(L, LUA_REGISTRYINDEX, c->pkey_ref);
	sprintf(s, "%s %p %s %s %d %p", LUACRYPTO_OPENNAME, (void *)c,
		EVP_CIPHER_name(c->cipher),
		(*pkey)->type == EVP_PKEY_DSA ? "DSA" : "RSA",
		EVP_PKEY_bits(*pkey), pkey);
...
static int open_update(lua_State * L)
{
	...
	while (input_len > 0) {
		...
		if (!EVP_OpenUpdate
		    (c->ctx, temp, &output_length, input, (int)sz)) {
			return crypto_error(L);
		}
...
static int luacrypto_hex(lua_State * L)
{
	size_t i, len = 0;
	const unsigned char *input =
	    (unsigned char *)luaL_checklstring(L, 1, &len);
				       ...
				       const luaL_Reg * methods)
{
	...
	lua_pushliteral(L,
			LUACRYPTO_PREFIX
			"you're not allowed to get this metatable");
	lua_settable(L, -3);

	return 1;
}

static void create_call_table(lua_State * L, const char *name,
			      lua_CFunction creator, lua_CFunction starter)
{
...
#define CALLTABLE(n) create_call_table(L, #n, n##_fnew, n##_f##n)
	...
	luacrypto_register_submodule(L, LUACRYPTO_X509_CERT_NAME,
				     x509_functions);
	luacrypto_register_submodule(L, LUACRYPTO_X509_CA_NAME,
				     x509_ca_functions);
